...
const stickers: { category: string; items: { src: string; label: string }[] }[] = [
  {
    category: 'Icônes',
    items: [
...
export default function StickersPanel({ onPick }: { onPick: (url: string) => void }) {
  return (
    <div className="card p-4 space-y-4 max-h-[520px] overflow-auto">
      <div className="font-semibold">Bibliothèque</div>
      {stickers.map((cat) => (
        <div key={cat.category} className="space-y-3">
          <div className="text-xs uppercase tracking-wide text-slate-500">{cat.category}</div>
          <div className="grid grid-cols-4 gap-3">
            {cat.items.map((s) => (
              <button key={s.src} onClick={()=>onPick(s.src)} className="rounded-lg border p-2 bg-white hover:shadow-md hover:-translate-y-0.5 transition-all">
                <Image src={s.src} alt={s.label} width={80} height={80} className="w-full h-16 object-contain" />
              </button>
            ))}
          </div>
        </div>
